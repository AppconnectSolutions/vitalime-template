...
const navItems = navbarData
  ? [
      { label: navbarData.label1, to: "/" },
      { label: navbarData.label2, to: "/products" },
      { label: navbarData.label3, to: "/shopnow" },
      { label: navbarData.label4, to: "/about" },
      { label: navbarData.label5, to: "/contact" },
      { label: navbarData.label6, to: "/combos" },
    ]
  : [];
...
const fetchOffer = async () => {
  try {
    const res = await fetch("http://localhost:5000/api/topOffer");
...
const fetchNavbar = async () => {
  ...
  <div className="container d-flex flex-column flex-md-row align-items-center justify-content-between">
    ...
    <div className="offer-timer text-center text-md-start mb-2 mb-md-0 flex-grow-1">
      <h6 className="mb-0">{offer.lemonMessage}</h6>

    </div>

    {/* RIGHT WHATSAPP */}
    <a
      href={`https://wa.me/${offer.whatsappNumber}`}
    ...
    >
      ...
      <span className="d-none d-sm-inline">Need help? Call Us: {offer.whatsappNumber}
</span>
    </a>
  ...
  src={
    navbarData?.logo_url
      ? `http://localhost:5000${navbarData.logo_url}`
      : "/assets/images/vita_logo.svg"
  }
              ...
              >
              <span style={{ color: "#086835fb" }}>
  {navbarData?.company_name || "Vitalimes"}
</span>
