      ...
      <nav className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <ul className="hidden md:flex items-center gap-8">
          ...
          <DropdownMenu
            ...
            items={[
              { label: "Manage Products", to: "/admin/manage/products" },
              { label: "Manage Categories", to: "/admin/manage/categories" },
              { label: "Manage Orders", to: "/admin/manage/orders" },
              { label: "Manage Users", to: "/admin/manage/users" },
              { label: "Roles & Access", to: "/admin/manage/roles" },
              { label: "Offers & Discounts", to: "/admin/manage/offers" },
              { label: "Reports", to: "/admin/manage/reports" },
              { label: "Settings", to: "/admin/manage/settings" },
            ]}
          />
        </ul>
      ...
      {open && (
        <div className="md:hidden border-t bg-white w-full">
          ...
          <MobileGroup title="Management">
            ...
            />
            <MobileItem to="/admin/manage/reports" label="Reports" />
            <MobileItem to="/admin/manage/settings" label="Settings" />
          </MobileGroup>
        </div>
      )}
    </header>
...
function DropdownMenu({ label, icon, items }) {
  return (
    <li className="group relative cursor-pointer">
      ...
      <div className="absolute left-0 top-full hidden group-hover:block bg-white border shadow-md rounded-md w-48 z-50">
        {items.map((item, idx) => (
          <NavLink
            key={idx}
            to={item.to}
